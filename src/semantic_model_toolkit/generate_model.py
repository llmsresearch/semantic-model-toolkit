...
def generate_description_with_llm(context: str, entity_type: str, entity_name: str, llm_config: Optional[LLMConfig] = None) -> str:
    """
...
def _raw_table_to_semantic_context_table(
    database: str, schema: str, raw_table: data_types.Table, llm_config: Optional[LLMConfig] = None
    ...
    for col in raw_table.columns:
        # Prepare context for LLM description generation
        column_context = f"Table: {raw_table.name}\nColumn: {col.column_name}\nData Type: {col.column_type}\nSample Values: {', '.join(str(v) for v in col.values[:3]) if col.values else 'None'}"
        ...
        if col.column_type.upper() in TIME_MEASURE_DATATYPES:
            description = col.comment if col.comment else generate_description_with_llm(
                column_context, "time dimension column", col.column_name, llm_config
            )
            ...
            description = col.comment if col.comment else generate_description_with_llm(
                column_context, "dimension column", col.column_name, llm_config
            )
            ...
            description = col.comment if col.comment else generate_description_with_llm(
                column_context, "measure column", col.column_name, llm_config
            )
            ...
            description = col.comment if col.comment else generate_description_with_llm(
                column_context, "general column", col.column_name, llm_config
            )
    ...
    table_description = raw_table.comment if raw_table.comment else generate_description_with_llm(
        table_context, "table", raw_table.name, llm_config
    )
